...
class TableHeader extends React.Component {
    ...
    render() {
        let {isMyAccount, settings} = this.props;
        let preferredUnit = settings ? settings.get("unit") : "1.3.0";

        return (
          <thead>
          <tr>
            <th style={leftAlign} width="25%"><Translate content="exchange.description" /></th>
            <th style={leftAlign} width="17%"><Translate content="exchange.price" /></th>
            <th style={leftAlign} width="17%"><Translate content="exchange.price_market" /></th>
        ...
        return  (
            <tr key={order.id} className="clickable">
                ...
                {isMyAccount ? <td className="text-center" style={{ padding: "2px 5px"}}>
                    {isCall ? null : <button onClick={this.props.onCancel} className="button tiny fillet"><Translate content="wallet.cancel"/></button>}
                </td> : <td>-</td>}
            </tr>
        );
    }
